...
- `content_bank/` — the human-reviewed content library (briefs, drafted items, store) and its authoring pipeline (`content_bank/author/`, prompt builders + gates + store writer). See `docs/content_builder_usage.md` (how to run the `build_cli.py` builder), `docs/content_translator_usage.md` (the `translate_cli.py` Chinese translator), and `docs/content_build_terminology.md` (pipeline glossary — pericope/section, D1–D8, throughline/thread, gates, citation tags, r1/r2 review, provenance).
